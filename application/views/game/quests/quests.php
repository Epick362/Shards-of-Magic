<div class="row-fluid">
	<div class="offset2 span8">
		<div class="quests default table">
					...
					if($quest['status'] == 1) {
						...
						for ($i = 1; $i <= 4; $i++) {
							if(array_key_exists('ReqCreatureName'.$i, $quest)) {
								echo '<ul class="unstyled"><li><strong>'.$quest['ReqCreatureName'.$i].' slain</strong> <em>'.$quest['ReqCreatureDone'.$i].'/'.$quest['ReqCreatureCount'.$i].'</em></li></ul>';
							}
						}
						for ($i = 1; $i <= 4; $i++) {
							if(array_key_exists('ReqItemName'.$i, $quest)) {
								echo '<ul class="unstyled"><li><strong>'.$quest['ReqItemName'.$i].'</strong> <em>'.$quest['ReqItemDone'.$i].'/'.$quest['ReqItemCount'.$i].'</em></li></ul>';
							}
						}
						echo '</div>';
					}
